			...
			.ready(
					function() {
						...
						function setCalendar() {
							...
							$("#startdate").attr("max",
									toDay2.substr(0, 8) + maxdd.substr(6));
							//조회끝날짜 속성 설정
							...
							console.log("이게 뭐시다냐 ==> " + toDay2.substr(0, 8)
									+ maxdd.substr(6));
							...
							$("#searcWeatherBtn")
									.click(
											function() {
												var start_date = parseInt($(
														"#startdate").val()
														.replace(/\-/g, "")); //"-"문자를 모두제거하는 정규식
												...
												if (start_date > end_date) {
													...
													searchMidweather(
															String(start_date),
															String(end_date)); //중기예보만호출
												} else {
													alert("All!");
													searchAllweather(
															String(start_date),
															String(end_date)) //모두호출
												}
											});
						}
						;
						...
						function main(data) {
							...
							if (data.length != 0) { //최초 접속 시 api 데이터가 성공적으로 전달될 때
								...
								for (let i = 0; i < data.list.length; i++) {
									...
									formHtml += "<div class=POP, id=POP>강수확률 : "
											+ data.list[i].pop + "%</div>"; //강수확률
									formHtml += "<div class=TMN, id=TMN>최저기온 : "
											+ data.list[i].tmn + "℃</div>"; //최저기온
									formHtml += "<div class=TMX, id=TMX>최고기온 : "
											+ data.list[i].tmx + "℃</div>"; //최고기온
									formHtml += "</div>";
								}
